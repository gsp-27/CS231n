...
class ThreeLayerConvNet(object):
  ...
  def loss(self, X, y=None):
    ...
    bn_params1 = bn_params2 = bn_params3 = {'mode':'train'}

    # defining helper function and enclosing conv_bn_relu in one
    def conv_bn_relu_f(X, W, b, gamma, beta, conv_param, bn_param):
      a1, c1 = conv_forward_fast(X, W, b, conv_param)
      a2, c2 = spatial_batchnorm_forward(a1, gamma, beta, bn_param)
      out, c3 = relu_forward(a2)
      cache = (c1, c2, c3)
      return out, cache

    def conv_bn_relu_b(dout, cache):
      c1, c2, c3 = cache
      da3 = relu_backward(dout, c3)
      da2, dgamma, dbeta = spatial_batchnorm_backward(da3, c2)
      da1, dW, db = conv_backward_fast(da2, c1)
      return da1, dW, db, dgamma, dbeta 
    ...
    # variable.                                                                #
    ############################################################################
    a1, c1 = conv_bn_relu_f(X, W1, b1, gamma1, beta1, conv_param, bn_params1)
    a2, c2 = max_pool_forward_fast(a1, pool_param)

    # second layer convolution and max pool
    a3, c3 = conv_bn_relu_f(a2, W2, b2, gamma2, beta2, conv_param, bn_params2)
    a4, c4 = max_pool_forward_fast(a3, pool_param)

    # for the fully connected layer
    a5, c5 = conv_bn_relu_f(a4, W3, b3, gamma3, beta3, fc_param, bn_params3)
    ...
    da5, dw3, db3, dgamma3, dbeta3 = conv_bn_relu_b(da6, c5)
    da4 = max_pool_backward_fast(da5, c4)
    da3, dw2, db2, dgamma2, dbeta2 = conv_bn_relu_b(da4, c3)
    da2 = max_pool_backward_fast(da3, c2)
    da1, dw1, db1, dgamma1, dbeta1 = conv_bn_relu_b(da2, c1)
