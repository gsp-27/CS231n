...
class KNearestNeighbor:
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    new_x = np.sum(np.square(X),axis=1).reshape(X.shape[0],1)
    new_x_train = np.sum(np.square(self.X_train),axis=1).reshape(1, self.X_train.shape[0])
    new_x_train = np.tile(new_x_train, (X.shape[0],1))
    new_x = np.tile(new_x, (1,self.X_train.shape[0]))
    dists = np.sqrt(new_x + new_x_train - 2 * X.dot(self.X_train.T))
    #########################################################################
    #                         END OF YOUR CODE                              #
    #########################################################################
    return dists
